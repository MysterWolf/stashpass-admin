...
| 2026-06-22 | Dominance field — Added `dominance` to `Strain` type and `StrainEdit.tsx`. Select dropdown in Basic section below Type chips. Five values: true_sativa / sativa_dominant / balanced / indica_dominant / true_indica; nullable (unset). |
| 2026-06-22 | Strain discovery queue — StrainQueue.tsx page at /strains/queue; table of pending/enriching rows sorted by surface_count (bold teal badge); Enrich (→ sets enriching status + navigates to StrainEdit pre-filled from queue row) and Reject (confirm + sets rejected) actions. StrainEdit auto-publishes queue row on save when launched from Enrich. Nav: "Strain Queue" (🔍) added to Sidebar + BottomNav after Strains. |
| 2026-07-08 | AI enrichment — Enrich button calls POST /ai/enrich-strain via enrichStrain() in src/api/strains.ts. Sets queue status to 'enriching', calls API, navigates to /strains/new with enriched data in router state. StrainEdit reads location.state.enriched and merges all fields into initial form state. Teal "AI-enriched" banner shown when enriched data present. Revert-to-pending on failure. |
| 2026-07-26 | Dominance in enrichment — added dominance to EnrichedStrain interface (src/types/index.ts) and to the enriched merge block in StrainEdit.tsx. Dominance dropdown now pre-fills from AI response. |
